...
JDK 17+ (25+ only for `enableAotCache`), Kotlin 2.0+, Gradle 8.0+. Nucleus 2.5.4 pairs with Kotlin 2.4.10 / Compose 1.12.0. Node.js is required for installer packaging (electron-builder pipeline).
...
1. **Version catalog**: `nucleus = "2.5.4"` version; libraries `dev.nucleusframework:nucleus.{nucleus-application, core-runtime, decorated-window-tao, decorated-window-material3, menu-macos}`; plugin alias `nucleus = { id = "dev.nucleusframework", version.ref = "nucleus" }`. ArtifactId convention: `nucleus.<module-dir>` (hence the double `nucleus.nucleus-application`).
