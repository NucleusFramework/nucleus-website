...
description: Embed any platform-native view — NSView, HWND, or GtkWidget — inside a Compose layout on the Tao backend.
---
...
The `content` slot works on **macOS, Windows, and Linux**:

- **macOS / Windows** — a sibling overlay surface (Metal NSView / DirectComposition HWND)
  hosts a separate Compose scene over the embedded native view.
- **Linux** — Compose draws the overlay in the main scene with alpha over the GTK widget
  buffer. Hit-test passthrough is region-based: outside
  `consumeOverlayPointerEvents` rects, clicks fall through to GTK and the embedded widget.
  From 2.3.1, overlay clicks use content-area coordinates (so CSD shadow margins do not
  offset hit-testing), and the full-surface opaque region is cleared while any `NativeView`
  is attached so GTK content (e.g. WebKit) shows through the EGL subsurface.
...
- [WebView](/docs/webview) — ready-made multiplatform WebView built on this primitive for desktop.
- [TextureView](/docs/tao/texture-view) — external GPU textures in the Compose scene (no CPU copy).
