...
file must use the `file://` scheme. On Windows, a local cover (`file://` or a bare path)
appears in the SMTC flyout; remote `http`/`https` URLs work as well.
...
`org.mpris.MediaPlayer2.<suffix>`, where `<suffix>` is `NucleusApp.appId` sanitized into a legal
D-Bus name); on macOS the identity comes
...
A local `coverUrl` (`file://` or a bare path) shows in the flyout; `http`/`https` URLs are
fetched by the system.

</Tab>
<Tab value="Linux">

`configure()` registers the MPRIS bus name `org.mpris.MediaPlayer2.<suffix>`, where `<suffix>` is
`NucleusApp.appId` sanitized into a legal D-Bus name: illegal characters become `_`, elements that
start with a digit get an `_` prefix, empty elements are dropped, and the suffix is truncated to
keep the name under the 255-character D-Bus limit. `playerctl` and desktop indicators discover the
player by enumerating that prefix. Pass a custom `dbusName` to override it.
...
- On Linux, `isAvailable()` only reports whether the native library loaded. If the session bus is unreachable or the MPRIS name is invalid, registration is skipped: `attach` returns without blocking and logs a warning, while `isAvailable()` still returns `true`.
