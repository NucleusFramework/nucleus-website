...
On Wayland, the portal persists grants by `(app_id, shortcut_id)`. From 2.3 the Linux path
derives a **stable** `shortcut_id` from modifiers + keyCode (`nucleus_m{mods}_k{keyCode}`) so
the same chord keeps the same id across launches and registration order — without that, each
start re-prompted the user.

`register(...)` calls are also **debounced** into a single `BindShortcuts` dialog. After a
loop of registrations, either wait for the flush or call `commitRegistrations()` to force it:
...
GlobalHotKeyManager.commitRegistrations() // one portal dialog for both

// Debug: portal id for a handle (Linux only; null elsewhere)
