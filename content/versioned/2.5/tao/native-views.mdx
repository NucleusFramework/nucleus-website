...
embedded view follows resizes and scrolls with no extra wiring. From 2.5.15, a trackpad
pan over a `NativeView` is consumed and forwarded (`PanStart` / `PanMove` / `PanEnd`) so
the embedded scroll view rubber-bands; the Compose scrollable above does not open a
competing pan session.
