...
|--------|------------------|------------------|
| **L1 — Metadata par lib** | 29 fichiers curés : Compose UI, Skiko, ktor, kotlinx.serialization, SQLite, androidx-sqlite-bundled, Coil, JNA, FileKit, SLF4J, la baseline JDK/AWT, et d'autres | Livrés dans le JAR du plugin. Les fichiers qui déclarent `matchPackages` ne sont inclus que si la lib correspondante est sur le classpath runtime. |
...
Le JAR `graalvm-runtime` enregistre sept globs de ressources dans la section `resources` de son
`reachability-metadata.json` :

| Glob | Couvre |
|-------|--------|
| `**/*.svg`, `**/*.ttf`, `**/*.otf` | Les icônes SVG et les polices du classpath — Jewel, icônes IntelliJ Platform, ressources Compose, et les vôtres |
...
| `META-INF/services/javax.sound.sampled.spi.MixerProvider` | Le descripteur du provider de mixage audio |
...
- [Chargeur de bibliothèques natives](/docs/performance/native-library-loader) — comment les libs JNI sous `nucleus/native/` sont chargées à l'exécution.
