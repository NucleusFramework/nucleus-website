...
set are covered without extra configuration. With `graalvm { headless = true }`, the always-on
GUI packs (`jdk-awt`, `jdk-fonts`, `jdk-graphics2d`, Skiko, Compose, tray, media player,
webview) are skipped even when they have no `matchPackages` filter. See
...
script. With `graalvm { headless = true }`, the AWT/Java2D platform file is not copied; only
the `main` class reflection entry is written.
...
- a supertype other than `Object` (or an interface) is referenced by app code.
...
- A `reachability-metadata.json` whose `resources` section registers the globs `**/*.svg`,
  `**/*.ttf`, `**/*.otf`, `composeResources/**`, `nucleus/**`, and `META-INF/services/**`.
- A `native-image.properties` carrying only `-H:+SharedArenaSupport`, which has no metadata
  equivalent.
...
From 2.3.1, well-known Kotlin tracing-agent phantoms (`kotlin.Any` / `Int` / `FunctionN` /
