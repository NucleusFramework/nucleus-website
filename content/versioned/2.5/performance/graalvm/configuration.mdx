...
Default is a GUI image: Nucleus registers AWT/Java2D/Skiko reachability metadata and copies
companion native libs (`libawt`, `libfontmanager`, Skiko, …) next to the binary. JNI
registration of AWT types then makes `native-image` pull `libawt` / `libawt_xawt` even when
app code never references `java.awt`.

For a daemon or CLI, set `headless = true`:
...
- Level 3 platform metadata (`sun.awt.windows.*` / `sun.lwawt.macosx.*` / `sun.awt.X11.*`,
  Java2D pipelines, font managers). The `main` class reflection entry is still written.
- Always-on Level 1 packs: `jdk-awt`, `jdk-fonts`, `jdk-graphics2d`, Skiko, Compose, tray,
  media player, webview.
- Copying companion GUI native libs into the packaged output.
- Bakes `-Djava.awt.headless=true` into the image.

Leave it `false` for any app that opens a window, a tray, or otherwise needs AWT/Skiko.
...
| `version` | `Property<String>` | unset | Overrides `channel`. Accepts `"25i3"`, `"25"`, or a pinned `"25.0.1"`. From 2.5.1, `INNOVATION` resolves to GraalVM **25.3** (`25i3`). |
...
| `headless` | `Property<Boolean>` | `false` | Daemons/CLIs: skip GUI metadata and native libs. See [Build a headless image](#build-a-headless-image). |
...
| `mlProfileInference` | `Property<Boolean>` | `true` | `false` emits `-H:-MLProfileInference` on **Oracle** toolchains only. On Community, the flag is unrecognized and is skipped with a warning (CE already has no ML pass). |
...
| `autoIncludeResources` | `Property<Boolean>` | `true` | Bundle the project's own resources (and `project(...)` module resources) into the image. |
| `maxHeapSizePercent` | `Property<Int>` | `25` | Default max heap as a percent of physical RAM. Follows the selected collector (`MaximumHeapSizePercent` for Serial/Epsilon, `MaxRAMPercentage` for G1). |
| `maxHeapSize` | `Property<String>` | unset | Absolute max heap (e.g. `"2g"`, `"512m"`). Takes precedence over `maxHeapSizePercent`. |
| `garbageCollector` | `Property<NativeImageGarbageCollector>` | unset | Bakes `--gc=` (`SERIAL`, `G1`, `EPSILON`). Unset keeps native-image Serial. `G1` is Oracle GraalVM + Linux only and degrades to a warning elsewhere. |
| `exactReachabilityMetadata` | `Property<ExactReachabilityMetadata>` | `APP_PACKAGES` | Dev-loop only (`runGraalvmNative`). See [Exact reachability](#exact-reachability-on-the-dev-loop). |
| `detectOrphanProjectClasses` | `Property<Boolean>` | `true` | Register public no-arg `<init>` for project classes only loaded by naming convention. See [Automatic metadata](/docs/performance/graalvm/automatic-metadata#detect-orphan-project-classes). |
| `reflectionForProjectClasses` | `Property<Boolean>` | `false` | Opt-in sledgehammer: every project class with a public no-arg constructor. |
...
## Exact reachability on the dev loop

`exactReachabilityMetadata` enables GraalVM's `--exact-reachability-metadata` on
**`runGraalvmNative` only** (the quick-build loop). Unregistered reflective lookups then throw
`MissingReflectionRegistrationError` naming the missing element, instead of a nested
`ClassNotFoundException` chain. Distributable tasks
(`createGraalvmNativeDistributable`, `packageGraalvmNativeDistributionForCurrentOS`,
`runGraalvmNativeDistributable`) never get the flag — optional-dependency probes in third-party
code would stop taking their fallback path under exact mode.

```kotlin title="build.gradle.kts"
graalvm {
    // Default: scope to the package of mainClass (and subpackages)
...
At run time, pass `-Pnucleus.graalvm.missingRegistration=warn|exit|throw` (default `warn`) so one
dev run surfaces every missing registration. Pre-JDK 23 toolchains degrade to
`-H:ThrowMissingRegistrationErrors=` or skip with a warning.

## Choose a garbage collector

The collector is fixed at build time for native images. Leave it unset for Serial (the right
default for a desktop-sized heap). Set G1 only when the heap outgrows what Serial can collect
without visible pauses, and only on Oracle GraalVM Linux:

```kotlin title="build.gradle.kts"
nucleus.application {
    // HotSpot JVM distribution (launcher + run task)
    garbageCollector = GarbageCollector.Z

    graalvm {
        // Baked into the native image (--gc=)
...
|-------------------------------|------|-------|
| `SERIAL` | `--gc=serial` | Default. Small heaps, pauses grow with heap size. |
| `G1` | `--gc=G1` | Oracle GraalVM + Linux only. Elsewhere: warning + Serial. |
| `EPSILON` | `--gc=epsilon` | No reclamation — benchmarks / short-lived processes only. |

JVM-side `GarbageCollector` values: `SERIAL`, `PARALLEL`, `G1`, `Z`, `SHENANDOAH`, `EPSILON`.
Flags are prepended so an explicit `-XX:+Use…GC` in `jvmArgs` still wins. Unset keeps the
JDK's own ergonomics.
...
| Argument | Purpose |
|----------|---------|
| `-Djava.awt.headless=false` | Enables GUI support. Prefer `graalvm { headless = false }` (the default) over this raw flag; use `headless = true` for daemons. |
| `-H:-IncludeMethodData` | Drops method metadata, reducing binary size by several MB. |

Nucleus does **not** force SLF4J to initialize at build time. The API and the app-selected
backend both initialize at run time, so provider discovery, log levels, and environment-dependent
configuration stay under app control. Forcing `--initialize-at-build-time=org.slf4j` from a
shared runtime module parks SLF4J 2.x backend objects in the image heap while their classes stay
run-time initialized. If you have a fixed, tested backend and want build-time init, opt in
explicitly:
...
2. Commit the `.iprof` file. Later full builds (`createGraalvmNativeDistributable`,
   `runGraalvmNativeDistributable`, packaging tasks) apply it automatically as
   `--pgo=<profile>`. The quick-build `runGraalvmNative` path skips PGO and advanced
   obfuscation.
...
On macOS (arm64), native images on the default Tao backend build on the auto-provisioned
toolchain (GraalVM CE by default). The deprecated AWT backend still requires BellSoft Liberica
NIK. Intel Macs fall back to Liberica NIK.
...
`packageReleaseGraalvmNative` and no `runReleaseGraalvmNative`. The public native tasks
follow the JVM pipeline (`runGraalvmNative`, `createGraalvmNativeDistributable`, …). This
is intentional:
