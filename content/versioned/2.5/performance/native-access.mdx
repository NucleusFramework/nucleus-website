...
|--------|----------------|----------------|
| **L1 — Per-library metadata** | 29 curated files: Compose UI, Skiko, ktor, kotlinx.serialization, SQLite, androidx-sqlite-bundled, Coil, JNA, FileKit, SLF4J, the JDK/AWT baseline, and more | Shipped in the plugin JAR. Files that declare `matchPackages` are included only when that library is on the runtime classpath. |
...
The `graalvm-runtime` JAR registers seven resource globs in the `resources` section of its
...
| `META-INF/services/javax.sound.sampled.spi.MixerProvider` | The audio mixer provider descriptor |
...
GTK4, …), write that side in Kotlin/Native and let [NucleusNativeAccess](/docs/performance/native-code)
...
- [Native code in Kotlin](/docs/performance/native-code) — call Kotlin/Native from the JVM through a generated FFM bridge.
- [Native library loader](/docs/performance/native-library-loader) — how JNI shared libraries under `nucleus/native/` are loaded at runtime.
