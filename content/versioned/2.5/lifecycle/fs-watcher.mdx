...
`ReadDirectoryChangesW`, and Linux uses inotify, all reached through a single JNI bridge.
Polling is opt-in via `FsWatchBackendStrategy.Polling` and runs inside that same native library.
There is no JVM-side fallback: when the native library is unavailable, `isSupported()` returns
`false` and `create()` throws `FsWatchException` for every strategy.
...
On macOS, FSEvents always reports **canonical** paths. From 2.3, a root reached *through* a
symlink (for example `Files.createTempDirectory()` under `/var` → `/private/var`) is treated as
the same directory for event routing — matching no longer depends on `followSymlinks`. Without
that, watches on temp dirs silently never fired.
...
- `FsWatchBackendStrategy.Auto` — the native backend. Currently identical to `NativeOnly`; neither degrades to polling.
