...
`ReadDirectoryChangesW` et Linux utilise inotify, tous atteints via un unique pont JNI. Le
polling s'active explicitement via `FsWatchBackendStrategy.Polling` et tourne dans cette même
bibliothèque native. Il n'y a pas de repli côté JVM : si la bibliothèque native est indisponible,
`isSupported()` renvoie `false` et `create()` lève `FsWatchException` pour toutes les stratégies.
...
Sous macOS, FSEvents rapporte toujours des chemins **canoniques**. Depuis 2.3, une racine
atteinte *via* un symlink (par exemple `Files.createTempDirectory()` sous `/var` →
`/private/var`) est traitée comme le même répertoire pour le routage d'événements — l'appariement
ne dépend plus de `followSymlinks`. Sans cela, les watches sur les temp dirs ne tiraient jamais.
...
- `FsWatchBackendStrategy.Auto` — le backend natif. Actuellement identique à `NativeOnly` ; aucun des deux ne se replie sur le polling.
