...
- If the lock file is stale or unreadable, `SingleInstanceManager` deletes it and retries once. If
  the retry finds the lock still held, the process is a secondary and sends a restore request.
  Only an unrecoverable error makes it fail open and continue as the primary.
