...
`setHandler(args, onDeepLink)` reconciles the difference. On the AWT path, `installAwtAppleEventHandler` registers an Apple Events handler so the callback fires while the app is already running; on Windows and Linux, `setHandler` scans `args` for the first deep-link-shaped entry. From 2.2 it accepts both authority forms (`myapp://path`) and opaque scheme-only forms such as `myapp:?query=…` (common from `xdg-open` on Linux). Windows drive paths (`C:\…`, `D:/…`) are rejected so they never look like schemes. The cold-start URI is delivered to the handler once per process, so registering from a Compose scope that re-runs on recomposition does not re-fire it.
