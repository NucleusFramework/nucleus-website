...
4. Enregistre le résultat — `Success`, `Failure`, `Retry` ou `ConstraintsNotMet` — et, en cas de `Retry`, programme une seule réexécution environ 5 minutes plus tard.
...
| `retryPolicy(policy)` | `RetryPolicy.Linear` ou `RetryPolicy.ExponentialBackoff`. Enregistré sur la requête ; les backends de plateforme réessaient actuellement à délai fixe, quelle que soit la politique. |
...
- `TaskResult.Retry(message)` — programme une réexécution environ 5 minutes plus tard. La `RetryPolicy` configurée n'est pas encore appliquée par les backends de plateforme.
...
tir. Un déclenchement calendaire ou on-boot non satisfait incrémente le compteur de tentatives et programme une seule réexécution environ 5 minutes plus tard.

### SchedulerConfig

Chaque backend invoque l'exécutable courant, résolu via
`ProcessHandle.current().info().command()`. Si votre application démarre par un launcher
maison, dirigez le scheduler vers ce launcher :
...
La commande obtenue est `<executablePath> <executableArguments…> --nucleus-scheduler-run <taskId>`.

| Propriété | Type | Défaut | Comportement |
|---|---|---|---|
| `executablePath` | `String?` | `null` | Chemin absolu du programme invoqué par le scheduler de l'OS. `null` ou vide résout l'exécutable courant. |
| `executableArguments` | `List<String>` | `emptyList()` | Arguments insérés avant le drapeau `--nucleus-scheduler-run`. |

`executablePath` doit être absolu : les scripts wrapper générés vérifient que le fichier existe
toujours et désenregistrent la tâche s'il a disparu, par exemple après une désinstallation.

<Callout type="warn">
Renseignez les deux propriétés avant le premier appel à `enqueue`. Les valeurs sont figées dans
le plist, l'unit ou la tâche générés au moment de la mise en file : un changement ultérieur ne
s'applique qu'aux tâches planifiées ensuite.
</Callout>
