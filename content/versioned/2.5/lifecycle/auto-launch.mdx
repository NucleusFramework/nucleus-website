...
- The generated Linux unit is ordered after `gnome-session-x11-services-ready.target` as well as
  `graphical-session.target` (2.5.6). `graphical-session.target` alone is reached before
  `gsd-xsettings` publishes `Xft.dpi`, so a toolkit that samples the display scale once at startup
  froze at 1.0 and rendered a HiDPI app at half size when it was started at login. systemd ignores
  ordering on units it has not loaded, so the dependency is inert outside GNOME.
