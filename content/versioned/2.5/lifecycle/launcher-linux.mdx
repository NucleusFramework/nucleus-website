...
The `.desktop` filename determines which launcher icon receives the signals. `launcher-linux`
never derives it for you — always pass it explicitly, as in
`LinuxLauncherEntry.appUri("myapp.desktop")`. (Auto-detection from the running process is a
[taskbar-progress](/docs/lifecycle/taskbar-progress) feature.)
