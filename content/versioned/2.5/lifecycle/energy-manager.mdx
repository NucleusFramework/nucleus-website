...
[Tao backend](/docs/tao) it now takes effect: while the modifier is attached, the display stays
awake.
...
Prefer it over the imperative calls for anything tied to a piece of UI: the request lives exactly
as long as the composable is in the composition, so there is no release to forget.
...
<Callout type="info">
The AWT backends do not implement the platform hook, so the modifier composes but has no effect
there.
</Callout>
