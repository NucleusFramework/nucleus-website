...
| `compressionLevel` | `CompressionLevel.Store`, `Normal`, `Maximum`, or `Ultra` (extra DMG/DEB recompression). |
...
<Callout type="warn">
`CompressionLevel.Maximum` and `Ultra` with AppImage cause very slow startup due to squashfs/FUSE decompression. Use `Normal` or `Store` for AppImage targets.
</Callout>
