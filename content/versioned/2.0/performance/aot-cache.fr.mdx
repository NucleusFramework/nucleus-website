...
`nucleus.aot-runtime` est tiré transitivement par [`nucleus.nucleus-application`](/docs/start/project-setup). Ajoutez-le explicitement uniquement si vous n'utilisez pas l'artefact umbrella :
...
- Si vous voulez une empreinte résidente plus petite ou un cold start plus court, utilisez plutôt la voie [GraalVM native image](/docs/performance/graalvm). L'AOT cache conserve le débit de HotSpot ; GraalVM échange un peu de débit contre le temps de démarrage et la taille.
