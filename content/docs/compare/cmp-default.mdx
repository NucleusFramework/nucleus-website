...
Sixteen formats against six. The plugin uses jpackage to build the app-image, then hands it to electron-builder's `--prepackaged` mode for everything else.
...
The plugin can compile your Compose Desktop app to a GraalVM Native Image and package the resulting binary as DMG, NSIS, or DEB. Cold start is around 0.5 s and RAM around 60 MB. The default CMP plugin does not handle Native Image.
...
- Decorated windows — Liquid Glass / Fluent / Yaru / Jewel, on JBR, stock JDK, or Tao.
...
- You want the 30+ OS integration modules without building them yourself.
