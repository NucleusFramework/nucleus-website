...
Chaque API est un wrapper typé sur l'appel Objective-C correspondant. `NotificationCenter` vit dans le process et transmet les callbacks `UNUserNotificationCenterDelegate` au `NotificationCenterDelegate` que on enregistres. Autorisation, catégories et notifications en attente sont interrogeables — pas d'état caché.
