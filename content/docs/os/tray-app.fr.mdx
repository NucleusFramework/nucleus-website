...
`TrayApp` ancre une fenêtre Compose complète à une icône du tray système : tu cliques gauche sur
l'icône et la fenêtre s'ouvre à côté, tu cliques ailleurs et elle se ferme. C'est le pattern menu
bar app utilisé par des outils comme Bartender, iStat Menus et Hidden Bar.

<Callout type="warn" title="Expérimental">
`TrayApp` est en alpha et l'API peut changer. Active-le avec `@OptIn(ExperimentalTrayAppApi::class)`.
</Callout>

## Ajouter la dépendance

`TrayApp` est inclus dans [`composenativetray`](/docs/os/system-tray). Si cette dépendance est sur
ton classpath, aucun artefact supplémentaire n'est nécessaire.

## Créer une tray app

Passe une icône, un tooltip, une taille de fenêtre et le contenu Compose à afficher dans la popup :
...
Conserve un `TrayAppState` pour afficher, masquer, redimensionner ou changer le mode de dismiss
...
Une tray app n'a généralement pas besoin d'icône dans le Dock. Sur macOS, mets `LSUIElement = true`
