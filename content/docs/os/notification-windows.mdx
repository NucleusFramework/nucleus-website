...
Unpackaged apps need a Start Menu `.lnk` shortcut whose AUMID property matches the one used at runtime. Without it, toasts may fail to appear or to persist in Action Center. `initialize` picks the policy automatically: `ShortcutPolicy.REQUIRE_CREATE` for installed apps and `REQUIRE_NO_CREATE` in dev mode, where no shortcut is created and a warning is logged. Pass `shortcutPolicy = ShortcutPolicy.REQUIRE_CREATE` to force it. The Nucleus Gradle plugin sets the shortcut up when you build the Windows installer formats.
</Callout>
