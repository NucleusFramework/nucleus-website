...
Add `google()` only when you pull Compose Multiplatform from there or share modules with
Android.
...
<Callout type="tip">
Nucleus extends the Compose plugin rather than replacing it, so Compose Hot Reload works
without extra setup. The `mainClass` you set in `nucleus.application` is propagated to
`compose.desktop.application.mainClass`, so the `hotRun` task resolves the entry point.
</Callout>
...
AOT runtime, the GraalVM runtime, and the default AWT window backend. Every other module is
opt-in.
