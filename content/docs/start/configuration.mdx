...
| `modules(...)`, `includeAllModules` | JDK modules to include. Default: `java.base`, `java.desktop`, `java.logging`, `java.net.http`, `jdk.accessibility`, `jdk.crypto.ec`, `jdk.unsupported`. |
...
| `compressionLevel` | `CompressionLevel.Store`, `Normal`, `Maximum`, or `Ultra` (extra DMG/DEB recompression). Overridable per format via `linux.appImage { compressionLevel }` and `windows.portable { compressionLevel }`. |
...
| `march` | `NativeImageMarch.COMPATIBILITY` (`NATIVE` on macOS arm64) | Target CPU instruction set. |
...
| `metadataRepository.version` | `"1.1.4"` | Repository version. |
...
<Callout type="warn">
`CompressionLevel.Maximum` and `Ultra` with AppImage cause very slow startup due to squashfs/FUSE decompression. Use `Normal` or `Store` for AppImage targets — set `linux.appImage { compressionLevel = CompressionLevel.Normal }` when the root level is higher for DEB/DMG.
</Callout>
