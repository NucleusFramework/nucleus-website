...
Utilise JDK 17 si l'on n'a pas besoin du cache AOT, JDK 25 pour l'activer (`enableAotCache = true` dans `nativeDistributions`). Pour les native images GraalVM, configure `nucleus.application.graalvm { javaLanguageVersion.set(25) }` — voir [Configuration](/docs/start/configuration).
