...
description: Écris le côté natif en Kotlin/Native et appelle-le depuis la JVM comme une simple API Kotlin. NucleusNativeAccess génère le bridge FFM, les proxies JVM et la metadata GraalVM pour toi.
---

import { Callout } from 'fumadocs-ui/components/callout';

Quand une API plateforme n'a aucun module runtime Nucleus (CoreGraphics, IOKit, Win32, GTK4, une bibliothèque C du système), tu écris ce côté-là en Kotlin/Native et tu l'appelles depuis la JVM comme une dépendance Kotlin normale. NucleusNativeAccess parse ton API `nativeMain`, génère le bridge C et les classes proxy JVM par-dessus la Foreign Function & Memory API (FFM), et embarque les bibliothèques partagées compilées dans ton JAR — pas de JNI écrit à la main, pas de boilerplate C.
...
## Écris une fois, appelle des deux côtés

Écris la classe native dans `nativeMain`. Seule l'API publique est exportée :
...
Appelle-la depuis `jvmMain` comme un objet Kotlin ordinaire. Le proxy est `AutoCloseable` ; le fermer libère l'objet natif :
...
Le plugin exécute un pipeline au moment du build sur tes sources natives :
...
Comme le côté natif est du Kotlin/Native, tu peux tirer une bibliothèque C avec `cinterop` et l'exposer à la JVM via le même bridge généré — c'est ainsi que tu atteins CoreGraphics, Win32, GTK4, ou n'importe quelle bibliothèque système depuis du code JVM :
...
Écris un fin wrapper Kotlin/Native par-dessus les bindings C dans `nativeMain`, et le plugin l'exporte vers la JVM comme n'importe quelle autre classe native.
