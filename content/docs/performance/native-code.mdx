...
Native access ships from [`NucleusFramework/NucleusNativeAccess`](https://github.com/NucleusFramework/NucleusNativeAccess) with its own release cycle, like the [system tray](/docs/os/system-tray), the [PDF reader](/docs/pdf), and the [WebView](/docs/webview). It is a Gradle plugin, `dev.nucleusframework.nna`, versioned independently from Nucleus and licensed MIT.
<Version module="nna" />
</Callout>
...
- [Native library loader](/docs/performance/native-library-loader) — how Nucleus modules load their own JNI libraries (separate from this plugin's `kne/` cache).
...
- [WebView](/docs/webview) — one WebView API across Android, iOS, web, and desktop.
