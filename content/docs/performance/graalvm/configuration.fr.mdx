...
Par défaut l'image est GUI : Nucleus enregistre la metadata de reachability AWT/Java2D/Skiko
et copie les libs natives compagnons (`libawt`, `libfontmanager`, Skiko, …) à côté du
binaire. L'enregistrement JNI des types AWT fait alors tirer `libawt` / `libawt_xawt` par
`native-image` même si le code de l'app ne référence jamais `java.awt`.

Pour un daemon ou une CLI, mettez `headless = true` :
...
- La metadata plateforme de niveau 3 (`sun.awt.windows.*` / `sun.lwawt.macosx.*` /
  `sun.awt.X11.*`, pipelines Java2D, gestionnaires de polices). L'entrée de réflexion de la
  classe `main` est tout de même écrite.
- Les packs de niveau 1 toujours inclus : `jdk-awt`, `jdk-fonts`, `jdk-graphics2d`, Skiko,
  Compose, tray, media player, webview.
- La copie des libs natives GUI compagnons dans la sortie empaquetée.
- Inscrit `-Djava.awt.headless=true` dans l'image.

Laissez `false` pour toute app qui ouvre une fenêtre, un tray, ou a besoin d'AWT/Skiko.
...
| `version` | `Property<String>` | non défini | Remplace `channel`. Accepte `"25i3"`, `"25"`, ou une version épinglée `"25.0.1"`. Depuis 2.5.1, `INNOVATION` résout GraalVM **25.3** (`25i3`). |
...
| `headless` | `Property<Boolean>` | `false` | Daemons/CLIs : saute la metadata et les libs natives GUI. Voir [Construire une image headless](#construire-une-image-headless). |
...
| Argument | Rôle |
|----------|------|
| `-Djava.awt.headless=false` | Active le support GUI. Préférez `graalvm { headless = false }` (le défaut) à ce flag brut ; utilisez `headless = true` pour les daemons. |
