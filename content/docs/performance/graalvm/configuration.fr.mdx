...
Mets `isEnabled = true` et choisis une toolchain. Une configuration minimale ressemble à ceci :
...
nucleus.application {
    ...
    graalvm {
        ...
        // La Java Toolchain de Gradle télécharge Liberica NIK 25 en local.
...
| `jvmVendor` | `Property<JvmVendorSpec>` | — | Mets `JvmVendorSpec.BELLSOFT` pour provisionner Liberica NIK. |
...
| Propriété | Type | Défaut | Notes |
|-----------|------|--------|-------|
| `enabled` | `Property<Boolean>` | `true` | Mets `false` pour ignorer entièrement le repository. |
...
| `sourceDir` | `DirectoryProperty` | `bin` de la toolchain | Répertoire d'où les DLL sont copiées. Pointe-le vers le MSVC redistributable si une DLL manque dans la toolchain. |
...
Utilise `-Os` dans `buildArgs` pour optimiser la taille plutôt que ProGuard.
...
plateforme. Tu n'as besoin de `nativeImageConfigBaseDir` que pour les entrées spécifiques à
...
- [Metadata automatique](/docs/performance/graalvm/automatic-metadata) — les cinq couches de metadata que Nucleus assemble pour toi.
