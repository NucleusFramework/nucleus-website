...
`native-image` discovers all of it from the classpath, so no configuration is needed. See
[Runtime bootstrap](/docs/performance/graalvm/runtime-bootstrap) for the substitution details.
...
- [Runtime bootstrap](/docs/performance/graalvm/runtime-bootstrap) — the `graalvm-runtime` module that ships Level 5.
