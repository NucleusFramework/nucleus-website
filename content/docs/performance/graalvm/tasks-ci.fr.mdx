...
Le plugin Gradle Nucleus enregistre un graphe de tâches pour compiler et empaqueter ton
...
| `cleanupGraalvmMetadata` | Retire de ton `reachability-metadata.json` manuel les entrées que la metadata automatique couvre déjà. |
...
Les quatre tâches de metadata s'exécutent en dépendance de `packageGraalvmNative`, tu
n'as donc que rarement à les invoquer directement. Les tâches `packageGraalvm<Format>`
utilisent electron-builder, qui exige Node.js sur le runner.

<Callout type="info">
Les tâches de packaging n'existent que pour les formats que tu demandes dans
...
binaire portable, mets `march = "compatibility"` dans le DSL :
...
<Callout type="info">
Les builds d'exemple Nucleus mappent `graalvm.march` sur une propriété Gradle pour que tu
puisses la surcharger en ligne de commande :
...
Le flag `-PnativeMarch` ne fonctionne que si ton script de build lit cette propriété.
</Callout>
...
pas possible. Lance une matrice sur `ubuntu-latest`, `macos-latest` et `windows-latest`,
et provisionne la toolchain avec l'action `setup-nucleus` :
...
Avec `graalvm: 'true'`, `setup-nucleus` provisionne BellSoft Liberica NIK (Java 25 par
...
les tâches de packaging electron-builder. Voir [CI/CD](/docs/packaging/ci-cd) pour le
...
Lance le binaire natif depuis un terminal. Les échecs de réflexion produisent des messages
clairs comme `ClassNotFoundException` ou `NoSuchFieldException`. Pour capturer les entrées
manquantes :

1. Lance `./gradlew runWithNativeAgent` et parcours le chemin de code défaillant pour que
   l'agent enregistre l'entrée.
2. L'agent fusionne sa sortie dans ta metadata sans écraser les entrées enrichies
...
- [Metadata automatique](/docs/performance/graalvm/automatic-metadata) — comment la metadata de réflexion est résolue pour toi.
