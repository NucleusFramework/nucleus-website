...
`nucleus.aot-runtime` est tiré transitivement par [`nucleus.nucleus-application`](/docs/start/project-setup). Ajoute-le explicitement si l'on n'utilises pas l'umbrella :
...
- Préfère la voie [GraalVM native image](/docs/performance/graalvm) si l'on veut un cold start sous 100 ms ou une empreinte résidente minuscule. L'AOT cache gagne sur le débit ; GraalVM gagne sur le démarrage et la taille.
