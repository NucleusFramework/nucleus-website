...
description: L'AOT cache de Project Leyden, généré automatiquement au build et embarqué dans ton installateur. Un flag Gradle, ~1 s de cold boot avec un JIT pleinement tiered prêt à compiler.
---

Les apps desktop JVM payaient une taxe de cold start : des milliers de classes chargées, du bytecode vérifié, des chemins chauds JIT-compilés avant que l'UI ne réponde. **C'est la raison originelle pour laquelle Nucleus existe.** JDK 25 corrige ça avec l'AOT cache single-step de [Project Leyden](https://openjdk.org/projects/leyden/) — classes, profils et code préchauffés hors ligne, rejoués au lancement. Nucleus génère ce cache pendant ton build et l'embarque dans l'installateur.

import { Callout } from 'fumadocs-ui/components/callout';

## TL;DR
- Un flag Gradle : `enableAotCache = true`.
- Le cold start tombe à ~1.2 s — HotSpot C2 reste ton JIT, mais le warmup disparaît.
- Le cache est généré par un run de ton app en **mode training** pendant le build, puis embarqué dans chaque distribution.
- `nucleus.aot-runtime` permet à ton code de brancher selon training vs runtime.

## Install

`nucleus.aot-runtime` est tiré transitivement par [`nucleus.nucleus-application`](/docs/start/project-setup). Ajoute-le explicitement si tu n'utilises pas l'umbrella :
...
1. Lancer ton app en **mode training** après `createDistributable`.
...
## Mode training — ton app doit s'auto-terminer

Pendant le training, la JVM observe un vrai run de ton app. Ton code doit sortir proprement pour que le build ne reste pas bloqué jusqu'au timeout de sécurité de 300 s. Avec `nucleusApplication { … }`, c'est une ligne :
...
Si tu appelles `application { … }` de Compose directement, gère le timer manuellement :
...
- Être représentatif d'une vraie session utilisateur — plus tu touches d'écrans réels, plus de méthodes sont profilées, plus le cold start post-cache est rapide.
...
Quand tu compares des chiffres de cold start, mesure depuis le spawn de process OS (`time ./MyApp` sous Linux/macOS, `Measure-Command` sous Windows) jusqu'au premier frame visible. Le chiffre de 1.2 s du landing vient d'un démarrage JBR 25 standard avec un AOT cache pré-chauffé par un run de training de 45 s qui parcourt l'écran d'accueil et un onglet réglages.

## Notes

- Le cache est par binaire — chaque release embarque un cache neuf. N'essaie pas de partager les caches entre versions.
- Préfère la voie [GraalVM native image](/docs/performance/graalvm) si tu veux un cold start sous 100 ms ou une empreinte résidente minuscule. L'AOT cache gagne sur le débit ; GraalVM gagne sur le démarrage et la taille.
