...
Nucleus génère cette metadata pour toi. Il collecte les entrées depuis cinq sources et les passe
...
Les tâches de build viennent du plugin Gradle Nucleus. Ajoute la baseline runtime pour que sa
...
L1 à L4 et ta propre config manuelle sont chacune passées comme une entrée
...
Ces motifs sont larges à dessein. Si tu tires la lib d'icônes IntelliJ Platform et que la taille
du binaire compte, surcharge le glob d'icônes avec un `resource-config.json` plus serré.
</Callout>
...
chargées dynamiquement. Pour ces cas, lance l'app une fois sous l'agent native-image de GraalVM :

```shell
./gradlew runWithNativeAgent
```

Parcours chaque écran et chaque fonctionnalité. L'agent enregistre les accès réflexion, JNI,
ressources et proxies dans un répertoire temporaire, puis fusionne les nouvelles entrées dans ta
config sans écraser celles que tu as enrichies à la main.

## Retirer la metadata manuelle redondante

Si tu as accumulé des entrées manuelles que les sources automatiques couvrent maintenant, élague-les :

```shell
./gradlew cleanupGraalvmMetadata
```

La tâche compare ton `reachability-metadata.json` à L1, L2, L3 et à la sortie de l'analyse
...
Si tu dépends de l'un d'eux, livre plutôt le [build AOT cache](/docs/performance/aot-cache).
...
Win32, GTK4, …), utilise [NucleusNativeAccess](https://github.com/NucleusFramework/NucleusNativeAccess),
un plugin séparé qui te laisse écrire le côté natif en Kotlin/Native et génère le bridge FFM pour
toi. Il émet `reflect-config.json`, `resource-config.json` et `reachability-metadata.json` pour
les proxies générés, que le pipeline GraalVM de Nucleus récupère de façon transparente.

L'ID du plugin est `dev.nucleusframeworknativeaccess`, versionné indépendamment de Nucleus. Voir
[son repository](https://github.com/NucleusFramework/NucleusNativeAccess) pour la table de mapping
