...
Les apps JVM embarquent les cacerts cuits dans leur JRE — invisibles aux racines d'entreprise, aux certificats installés par MDM, et aux proxies d'inspection SSL. Résultat : `SSLHandshakeException` sur chaque machine que ton IT a touchée. Les modules HTTP natifs de Nucleus enveloppent ton client préféré avec [`NativeTrustManager`](/docs/performance/native-ssl), donc HTTPS vers ces hôtes fonctionne sans câblage manuel du trust store.
...
`NativeHttpClient.create()` renvoie un `java.net.http.HttpClient` initialisé avec `NativeTrustManager.sslContext` et le suivi de redirections activé. `withNativeSsl()` est une extension de `HttpClient.Builder` quand tu veux contrôler le reste de la chaîne.

> Attention : les redirections sont **désactivées** par défaut sur `HttpClient.Builder`. Si tu utilises `withNativeSsl()` directement, appelle `.followRedirects(NORMAL)` toi-même — sinon GitHub Releases, S3, et tout CDN qui renvoie 302 ressembleront à une erreur.
...
Les JARs des moteurs sont `compileOnly` dans `native-http-ktor` — seul le moteur que tu déclares vraiment doit être sur le classpath runtime.

## Comment ça marche

Chaque module est un fin adaptateur au-dessus de `native-ssl`. À la première utilisation, `NativeTrustManager` parcourt paresseusement le trust store de l'OS (Keychain sous macOS, stores Crypt32 sous Windows, les chemins standards de bundles PEM sous Linux) et merge ces ancres avec les défauts JVM. Le résultat est un seul `X509ExtendedTrustManager` qui accepte à la fois le programme racine Apple/Microsoft et tout ce que ton IT ou ton utilisateur a ajouté. Voir [SSL natif](/docs/performance/native-ssl) pour les détails par plateforme.

Les modules ne remplacent pas ton client HTTP — ils remplacent son trust manager. Connection pooling, HTTP/2, interceptors, timeouts : tout reste à toi.
