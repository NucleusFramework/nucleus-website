...
`NativeHttpClient.create()` renvoie un `java.net.http.HttpClient` initialisé avec `NativeTrustManager.sslContext` et le suivi de redirections activé. `withNativeSsl()` est une extension de `HttpClient.Builder` quand on veux contrôler le reste de la chaîne.

> Attention : les redirections sont **désactivées** par défaut sur `HttpClient.Builder`. Si on utilises `withNativeSsl()` directement, appelle `.followRedirects(NORMAL)` toi-même — sinon GitHub Releases, S3, et tout CDN qui renvoie 302 ressembleront à une erreur.
...
Les JARs des moteurs sont `compileOnly` dans `native-http-ktor` — seul le moteur que on déclares vraiment doit être sur le classpath runtime.
...
Les modules ne remplacent pas le client HTTP — ils remplacent son trust manager. Connection pooling, HTTP/2, interceptors, timeouts : tout reste à toi.
