...
Passe-les à le client HTTP. Si ce client est `java.net.http`, OkHttp ou Ktor, utilise plutôt les adaptateurs [HTTP natif](/docs/performance/native-http) — ils s'occupent du câblage.
...
| `interface NativeCertificateProvider` | SPI si on veux brancher une source de certs custom |
...
Les classes du bridge JNI doivent être préservées sous macOS et Windows. Le plugin Gradle Nucleus ajoute les règles automatiquement ; si on maintiens la config ProGuard à la main :
