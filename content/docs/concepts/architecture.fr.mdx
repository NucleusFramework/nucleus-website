...
- Tout ça tourne sur **JDK 17+, JBR ou GraalVM Native Image** — choisis le runtime dont on as besoin.

## Les couches

### Haut — le application

Compose UI, view-models, la logique métier. Du Kotlin pur. Si on veux partager avec Android et iOS, c'est cette couche que on mets sous un source set `commonMain` KMP.
...
Modules partagés, coroutines, `StateFlow`, `kotlinx.serialization`, Ktor. La même stack KMP que on utilises déjà pour Android et iOS. Les modules runtime Nucleus sont JVM-only, mais les couches au-dessus restent multiplatform.
