...
window backend on top, and `spellcheck` as an `api` dependency (Linux Hunspell; no-op
elsewhere). Everything else is opt-in.
...
| Spell check (Linux Hunspell) | [`spellcheck`](/docs/os/spellcheck) |
