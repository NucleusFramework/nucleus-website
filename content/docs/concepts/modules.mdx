...
This pulls in the runtime modules every app needs:

- `core-runtime` — platform detection, [native-library loading](/docs/performance/native-library-loader), the deep-link handler, the single-instance lock, and app metadata.
...
- `darkmode-detector` — reactive OS theme, exposed as `api` so `isSystemInDarkMode()` needs no extra dependency.
- `spellcheck` — the [native spell engines](/docs/os/spell-check), also exposed as `api`.

It also brings the shared Compose window code in `decorated-window-core`, so you add only a
window backend on top. `core-runtime` and `graalvm-runtime` are `implementation` dependencies —
present at run time, not on your compile classpath. Everything else is opt-in.
...
Full design systems (widgets + theme + optional window) live in **separate repositories**,
versioned independently:

| Design system | Window artifact | Repo |
|---------------|-----------------|------|
...
See [Native design systems](/docs/window/toolkits).

Pick exactly one backend (`-tao`, or one of the deprecated AWT modules). Style adapters and
external design systems are optional.
