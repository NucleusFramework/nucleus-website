...
[File dialogs](/docs/ecosystem/file-dialog) have no Nucleus module — use FileKit. The Nucleus
Gradle plugin ships GraalVM reachability metadata for FileKit, so it works under Native Image
with no manual configuration.

Spell check is first-party: wrap a field with [`SpellcheckContextMenu`](/docs/os/spellcheck).
