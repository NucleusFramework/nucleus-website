...
description: Importe des certificats de CA racines privées dans le JDK embarqué avec ton application empaquetée pour que ses appels HTTPS leur fassent confiance.
---

import { Callout } from 'fumadocs-ui/components/callout';

Nucleus peut importer des certificats de CA privées dans le keystore `cacerts` du JDK qu'il embarque
avec ton application empaquetée. Tu déclares les fichiers de certificat dans le DSL Gradle, et Nucleus
...
C'est utile quand ton application tourne derrière un proxy d'entreprise, une passerelle ZTNA ou un
...
Ajoute les fichiers de certificat à `nativeDistributions.trustedCertificates` :
...
s'exécute automatiquement pendant l'empaquetage ; tu ne l'invoques pas directement.
...
- `trustedCertificates` couvre les CA connues au moment du build, livrées à tous les utilisateurs. Pour
  ...
  macOS, CryptoAPI sur Windows, NSS ou stores PEM système sur Linux), utilise plutôt le
  [module `native-ssl`](/docs/performance/native-ssl).

<Callout type="info">
Les deux approches sont complémentaires. Livre les CA connues au build avec `trustedCertificates`, et
lis les CA déjà installées par l'utilisateur avec `native-ssl`.
</Callout>

## Et ensuite

- [Signature du code](/docs/packaging/code-signing) — signe et notarise l'application empaquetée.
- [native-ssl](/docs/performance/native-ssl) — fais confiance aux stores de certificats du système d'exploitation au runtime.
