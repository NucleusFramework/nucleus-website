...
- Linux : la signature GPG des repos DEB/RPM sort du périmètre ; tu signes avec les outils de ta distro.
...
- **Propriétés Gradle équivalentes.** Chaque champ de notarisation a une propriété Gradle `compose.desktop.mac.notarization.<name>` — utile quand tu préfères passer les secrets via `-P` plutôt qu'en variables d'env.
