...
description: Signez et notarisez les installateurs de votre app sur macOS, signez avec un certificat PFX ou Azure Artifact Signing sur Windows, et signez les paquets DEB/RPM en GPG sur Linux.
---
...
La signature est livrée avec le plugin Gradle, il n'y a donc rien de plus à installer. Ce dont vous avez
...
plusieurs dans le même build est rejeté à la validation : choisissez-en un.
...
2. Un profil keychain `notarytool`. Stockez les identifiants une fois avec
   `xcrun notarytool store-credentials AC_PASSWORD …`, puis référencez le profil :
...
Pour signer via un HSM cloud plutôt qu'avec un fichier de certificat local, utilisez Azure Artifact
Signing :
...
2. Activez la signature avec cet id de clé :
   ...
   La clé est lue depuis votre trousseau GPG local. En CI, pointez plutôt `keyFile` sur une clé
...
3. Buildez. Chaque paquet sort signé, avec sa clé publique écrite à côté :
   ...
   Publiez chaque paquet avec son `.pub.asc` (et le `.deb.asc` pour le DEB) sur votre page de
   téléchargement.
...
Linux ; voir [mises à jour sans mot de passe](/docs/packaging/auto-update#mettre-en-place-les-mises-à-jour-sans-mot-de-passe-sur-linux)
...
Ajoutez `LINUX_GPG_PRIVATE_KEY`, `LINUX_GPG_KEY_ID` et (si définie) `LINUX_GPG_PASSPHRASE`. Le
...
- **Propriétés Gradle équivalentes.** Chaque champ de notarisation a une propriété Gradle
  `compose.desktop.mac.notarization.<name>`, utile quand vous passez les secrets avec `-P` plutôt qu'en
...
- **Rotation du `.pfx`.** Mettez à jour le certificat au moins 30 jours avant son expiration. Un binaire
  signé mais expiré échoue SmartScreen jusqu'à ce qu'une nouvelle signature timestampée soit
  appliquée.

## Et ensuite

- [CI/CD](/docs/packaging/ci-cd) — signez les builds dans GitHub Actions avec les actions composites.
- [Mise à jour automatique](/docs/packaging/auto-update) — livrez des mises à jour signées, dont les mises à jour sans mot de passe sur Linux.
