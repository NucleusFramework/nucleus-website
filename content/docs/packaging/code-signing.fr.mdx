...
- Linux : la signature GPG des repos DEB/RPM sort du périmètre ; on signes avec les outils de la distro.
...
- **Propriétés Gradle équivalentes.** Chaque champ de notarisation a une propriété Gradle `compose.desktop.mac.notarization.<name>` — utile quand on préfères passer les secrets via `-P` plutôt qu'en variables d'env.
