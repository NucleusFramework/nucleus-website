---
title: Auto-update
description: Vérifie, télécharge, valide et installe les mises à jour de ton application depuis Kotlin, sans service tiers.
---

import { Callout } from 'fumadocs-ui/components/callout';

Le module `updater-runtime` vérifie, télécharge, valide et installe les mises à jour de ton
application depuis Kotlin. Il a une moitié build-time — les métadonnées `latest-*.yml` générées à
côté de tes installateurs — et une moitié runtime, la classe `NucleusUpdater`. Le format des
...
Crée un `NucleusUpdater`, interroge le provider configuré, puis télécharge et installe :
...
`checkForUpdates()` est une fonction `suspend` ; appelle-la depuis une coroutine.
...
initiale. Ajoute `TargetFormat.Zip` à côté de `TargetFormat.Dmg` pour que les deux soient dans le
...
- **S3** — renseigne les credentials via des variables d'environnement ; le plugin upload à côté du YAML.
- **HTTP générique** — upload toi-même sur n'importe quel host statique ; l'updater récupère `<baseUrl>/latest-*.yml`.
...
newVersion, updateLevel)` que tu peux utiliser pour une bannière « Quoi de neuf ».

## Configurer les réseaux d'entreprise

Si tes utilisateurs sont derrière un proxy d'entreprise avec une CA racine privée, le
`java.net.http.HttpClient` par défaut échoue au handshake TLS. Injecte un `NativeHttpClient` qui lit
...
de passe PolicyKit. Tu peux laisser l'app installer les mises à jour vérifiées sans mot de passe, en
...
2. Ta clé publique de signature, bundlée dans `/opt/<App>/resources/nucleus-update.pub.asc`.
...
donc pas installer un paquet arbitraire — il faudrait ta clé privée de signature. Si le helper ou le
...
`UpdateLevel` indique l'ampleur du saut : `MAJOR`, `MINOR`, `PATCH` ou `PRE_RELEASE`. Branche ton UI
...
- Pour le Mac App Store, le Microsoft Store, Snapcraft et Flathub, le store gère les mises à jour ;
  ignore `updater-runtime` entièrement.
