---
title: CI/CD
description: Construisez, signez et publiez une application desktop Nucleus sur macOS, Windows et Linux avec les actions GitHub composites livrées dans le dépôt Nucleus.
---

import { Callout } from 'fumadocs-ui/components/callout';

Dans ce tutoriel, vous mettez en place une pipeline de release GitHub Actions pour une application
desktop Nucleus. Vous construisez les installateurs sur une matrice par OS et par architecture, vous
fusionnez les builds macOS en un binaire universel, vous regroupez les builds Windows pour le
Microsoft Store, vous générez les métadonnées de mise à jour, puis vous publiez le tout dans une
release GitHub.

Nucleus livre sept actions composites dans le dépôt
[`NucleusFramework/Nucleus`](https://github.com/NucleusFramework/Nucleus). Vous les référencez par
leur chemin depuis votre propre workflow — il n'y a rien à copier dans votre projet.

## Avant de commencer

- Votre projet est hébergé sur GitHub et applique le plugin Gradle Nucleus. Voir
...
`NucleusFramework/Nucleus/.github/actions/<name>@<ref>`. Utilisez `@main` pendant vos essais et
épinglez un tag comme `@v2.5.10` pour la production.
</Callout>

## Référencer une action

Ajoutez une étape qui pointe vers une action du dépôt Nucleus :
...
| Action | Rôle |
|--------|------|
| `setup-nucleus` | Installe un JDK (Microsoft Build of OpenJDK sous Windows, Temurin ailleurs), les outils de packaging Linux, Gradle et Node. |
...
| `validate-release-ref` | Vérifie qu'un tag de release a bien été créé depuis la branche qui le porte. |

## Construire sur chaque plateforme

Créez `.github/workflows/release.yaml`. Déclenchez sur les tags de version, exécutez une matrice sur
les six runners, préparez l'environnement avec `setup-nucleus`, empaquetez pour l'OS courant et
téléversez le résultat en artefact :
...
Les jobs suivants téléchargent ces artefacts par leur nom `release-assets-<os>-<arch>` : conservez
...
| Entrée | Défaut | Rôle |
|--------|--------|------|
| `java-version` | `25` | JDK utilisé pour lancer Gradle et embarqué par jpackage. Un simple numéro majeur LTS résout le dernier build par OS. |
| `java-package` | `jdk` | Variante du package JDK (`jdk`, `jre`). |
...
| `graalvm` | `false` | Prépare un build native-image : outils de build natifs (Xcode/MSVC) et cache de toolchain. |
| `graalvm-version` | `25i3` | Version de la toolchain — composant de clé de cache uniquement. À garder synchronisé avec `graalvm { toolchain { } }`. |
| `graalvm-distribution` | `community` | Distribution de la toolchain — composant de clé de cache uniquement (`community` ou `oracle`). |
...
Le plugin Nucleus télécharge et met en cache le JDK GraalVM lui-même : `graalvm: 'true'`
n'installe donc jamais de JDK. L'action ne prépare que ce dont le plugin est incapable — elle
sélectionne Xcode 26 sur macOS, configure MSVC sous Windows via `ilammy/msvc-dev-cmd@v1` et met
en cache le répertoire de toolchain. Les deux entrées `graalvm-*` n'alimentent que la clé de
cache : gardez-les synchronisées avec votre bloc `graalvm { toolchain { } }` pour qu'un
changement de distribution ne restaure pas le mauvais cache.
...
<Callout type="warn">
Sur les dépôts **privés**, les runners macOS sont facturés avec un multiplicateur de 10× sur votre
quota mensuel de minutes, et `--wait` garde le runner actif pendant qu'il interroge le service de
notarisation d'Apple — vous payez ce temps d'attente au tarif macOS. Les dépôts publics sont
gratuits. Pour réduire les coûts sur un dépôt privé, exécutez les étapes de signature et de
...
puis applique le type de release. Passez `prerelease` pour marquer la release comme pré-release :
