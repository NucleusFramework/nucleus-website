...
épinglez un tag comme `@v2.4.0` pour la production.
</Callout>
...
| `graalvm` | `false` | Épingle une toolchain GraalVM (Liberica NIK) au lieu de l'Oracle GraalVM téléchargé automatiquement. |
...
le défaut — si bien que `graalvm: 'true'` n'est plus nécessaire pour la compilation native.
Ne l'activez que pour épingler votre propre toolchain : l'action installe alors Liberica NIK à
la place du JBR, exporte `GRAALVM_HOME` (qui est prioritaire sur le téléchargement automatique
lorsqu'il correspond à la distribution configurée), sélectionne Xcode 26 sur macOS et configure
MSVC sur Windows via `ilammy/msvc-dev-cmd@v1`. Les
toolchains de plateforme (Xcode, MSVC) restent requises pour les builds natifs dans tous les cas.
