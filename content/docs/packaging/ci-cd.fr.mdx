...
Construire, signer et publier sur trois OS et quatre architectures à la main, c'est un job à temps plein. Nucleus livre six actions composites GitHub Actions qui gèrent toute la pipeline — et un workflow de release de référence que on peux `uses:` directement depuis le repo Nucleus, sans copier-coller.
...
Lancer `gh release create` avec le tag de version, les installateurs assemblés et les YAML. Marque pre-release automatiquement pour les tags `*-alpha-*` / `*-beta-*`.
