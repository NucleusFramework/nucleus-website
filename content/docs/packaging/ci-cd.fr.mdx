...
Les builds natifs téléchargent et mettent en cache Oracle GraalVM automatiquement — le défaut
recommandé — si bien que `graalvm: 'true'` n'est plus nécessaire pour la compilation native.
...
sélectionne Xcode 26 sur macOS et configure MSVC sur Windows via `ilammy/msvc-dev-cmd@v1`. Les
