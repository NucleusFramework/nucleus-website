...
also paint over it, and `Popup`s from inside the scene land above the native view.
...
The native view is composited into the scene with a punched hole: Compose clears the view's
rectangle to transparent, and the platform shows the native content through it.
...
This requires Wayland. On the X11 backend a child window's alpha is never blended against its
parent, so the punched rectangle shows the desktop instead of the widget — the widget still runs,
but behind the GL surface.
</Tab>
</Tabs>
