...
Symptôme si on sautes : `Dependency resolution is looking for a library compatible with JVM runtime version 11, but 'dev.nucleusframework:nucleus.decorated-window-jewel' is only compatible with JVM runtime version 25 or newer.`
...
Si on déclares Jewel directement, bump-le sur la même coordonnée que Nucleus apporte transitivement :
...
En 1.x on emballais le `application { }` de Compose Desktop avec des helpers Nucleus. En 2.0 `nucleusApplication` fait le bootstrap dans le bon ordre et expose un `NucleusApplicationScope` unifié.
...
2. Timer de training AOT (si `-Dnucleus.aot.mode=training` et que on appelles `aotTraining(…)`).
...
Tout composable wrapper que on as écrit en 1.x doit devenir une extension sur le même scope :
