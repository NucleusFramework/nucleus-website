...
<Callout title="Prefer not to do this by hand?">
An AI coding agent can run this whole migration for you — including the window-shell and
AWT-removal work this build-focused guide doesn't cover. One prompt, zero install:
...
dependencies {
    ...
    implementation("dev.nucleusframework:nucleus.aot-runtime:<version>")           // if enableAotCache
    implementation("dev.nucleusframework:nucleus.updater-runtime:<version>")       // if publishing
    implementation("dev.nucleusframework:nucleus.taskbar-progress:<version>")
    implementation("dev.nucleusframework:nucleus.nucleus-application:<version>")   // for nucleusApplication { }
}
```
