...
- `beforeInstall: RegularFileProperty`, `beforeRemove: RegularFileProperty` — passed raw to fpm (`--before-install` / `--before-remove`); no electron-builder macro substitution. Pacman `pre_remove` has no `$1=upgrade` — stop/disable unconditionally and re-enable in `afterInstall`
