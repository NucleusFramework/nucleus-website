...
- `oneClick: Boolean` — skip the wizard. Default `true`. Set `false` for an assisted installer.
- `runAfterFinish: Boolean` — launch the app when the installer finishes. Default `true`.
- `createDesktopShortcut: Boolean` — default `true`.
- `createStartMenuShortcut: Boolean` — default `true`.
- `menuCategory: String?` — start-menu submenu (and Program Files subdirectory). Default `null`; when unset, `windows.menuGroup` is used.
- `shortcutName: String?` — shortcut label. Defaults to the application name.
...
- `afterInstall: RegularFileProperty`, `afterRemove: RegularFileProperty` — concatenated after Nucleus's template, then electron-builder. Macros (`${executable}`, `${sanitizedProductName}`) substitute **only when single-quoted**. Deb / Rpm / Pacman only. See [install and remove scripts](/docs/packaging/linux#add-install-and-remove-scripts)
- `beforeInstall: RegularFileProperty`, `beforeRemove: RegularFileProperty` — passed raw to fpm (`--before-install` / `--before-remove`); no electron-builder macro substitution. Not called on a Pacman upgrade — use `afterUpgrade` / `beforeUpgrade`
- `afterUpgrade: RegularFileProperty`, `beforeUpgrade: RegularFileProperty` — passed raw to fpm (`--after-upgrade` / `--before-upgrade`); no macro substitution. Pacman only: the only hooks that run when Pacman upgrades an installed package. Deb / Rpm ignore them
...
- `headless: Property<Boolean>` (default `false`) — daemons/CLIs: skip L3 AWT/Java2D platform metadata, always-on L1 GUI packs, and companion GUI native libs; bake `-Djava.awt.headless=true`. See [Build a headless image](/docs/performance/graalvm/configuration#build-a-headless-image)
