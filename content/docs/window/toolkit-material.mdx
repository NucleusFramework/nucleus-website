...
- `ApplicationScope.MaterialDecoratedWindow(...)` and `NucleusApplicationScope.MaterialDecoratedWindow(...)` — the `NucleusApplicationScope` overload also takes `nativePopupLayers`, `nativeContextMenu`, `hiddenFromDock`, `titleBarStyle`, `undecorated`, and the Tao-only flags `transparent`, `clickThrough`, `visibleOnAllWorkspaces` and `forceX11` (Material 2 has the same set minus `nativePopupLayers`)
