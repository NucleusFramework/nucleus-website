...
version you use. Pair it with `decorated-window-tao` for a runtime without AWT.
...
<Callout type="info">
The Material adapters run on every backend. Pair them with `decorated-window-tao` for a runtime
without AWT, or with `-jbr` / `-jni` when you have Swing dependencies.
</Callout>
