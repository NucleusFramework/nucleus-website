...
description: Exécute des tâches périodiques, calendaires et on-boot depuis Kotlin, que l'OS déclenche à l'heure même quand ton app est fermée.
---

import { Callout } from 'fumadocs-ui/components/callout';

Le module scheduler te permet d'exécuter des tâches en arrière-plan selon un horaire depuis
Kotlin, même quand ton app est fermée. Tu enregistres une tâche une fois et l'OS — launchd sur
...
il écrit des plists launchd au runtime, ce que la sandbox interdit. Utilise plutôt
...
`kotlinx-serialization-json` est exposé transitivement, donc tu peux annoter l'input d'une
tâche avec `@Serializable` sans l'ajouter toi-même.

## Planifier une tâche

### 1. Définir une tâche et un registry

Implémente `DesktopTask`, donne-lui un `TaskId`, et enregistre une factory. L'OS relance ton
binaire pour exécuter une tâche : le registry sert à retrouver ton code à partir d'un ID.
...
Quand l'OS déclenche une tâche, il relance ton binaire avec un flag scheduler. Vérifie-le tout
en haut de `main()` et passe la main à `DesktopBootReceiver`, qui exécute la tâche puis quitte.
...
Construis une `TaskRequest` avec l'une des trois factories et mets-la en file. L'enqueue est
...
binaires. Le scheduler enregistre donc le binaire de ton application auprès du système de
timer/job de la plateforme et passe `--nucleus-scheduler-run <taskId>` en ligne de commande.
Au déclenchement, ton `main()` tourne, `DesktopBootReceiver.isSchedulerInvocation(args)` renvoie
`true`, et le receiver :
...
3. Si elles sont satisfaites, appelle ton `doWork(context)`.
...
nettoyage des orphelins repose alors sur ton appel à `DesktopTaskScheduler.cancelAll()` dans tes
...
Récupère le singleton avec `DesktopTaskScheduler.getInstance()`, ou appelle les méthodes
...
Crée les requêtes avec les factories `periodic(taskId, interval)`, `calendar(taskId, expression)`
...
- Ne mets pas de secrets dans `inputData` — c'est persisté en JSON clair dans le fichier de
  métadonnées de la tâche. Stockes-y des références et résous les identifiants depuis le keychain
  au runtime.
- `CronExpression.everyHour()` signifie le top de l'heure en heure murale, pas « une heure après
  l'enqueue ». Pour une cadence fixe relative à l'heure d'enqueue, utilise
...
- [Auto-launch](/docs/lifecycle/auto-launch) — lancer ton app elle-même au login, pas seulement une tâche.
