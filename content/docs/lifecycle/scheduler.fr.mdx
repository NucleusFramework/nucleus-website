...
Un scheduler style WorkManager pour le desktop. Tu enregistres une tâche une fois et l'OS — launchd sur macOS, Task Scheduler sur Windows, timers systemd sur Linux — la déclenche à l'heure dite, même quand le app est fermée. Tu écris un `DesktopTask`, on déclares son trigger, la plateforme s'occupe des réveils et de la persistance.

## TL;DR
- Trois formes : `periodic(interval)`, `calendar(cronExpr)`, `onBoot()`.
- `DesktopBootReceiver.isSchedulerInvocation(args)` tout en haut de `main()` est le point d'entrée quand l'OS relance le binaire pour exécuter un job.
...
3. Si OK, appelle le `doWork(context)`.
...
- Pas de secrets dans `inputData` — c'est du JSON sur disque. Passe des références et résous les creds depuis le keychain au runtime.
