...
Double-clique deux fois sur ton app et la plupart des utilisateurs s'attendent à ce que le second clic ramène la fenêtre au premier plan, pas qu'il lance un second process. `SingleInstanceManager` en fait le comportement par défaut — et te donne un moyen propre de passer des arguments de l'invocation secondaire vers la primaire.

## TL;DR
- Fourni avec `nucleus-application` — `nucleusApplication { }` acquiert le verrou pour toi par défaut.
- Ou appelle `SingleInstanceManager.isSingleInstance(...)` à la main si tu n'utilises pas `nucleusApplication { }`.
...
À la main, quand tu câbles ton propre `application { }` :
    ...
    val isSingle = remember {
        SingleInstanceManager.isSingleInstance(
            onRestoreFileCreated = {
                ...
                // Écris ce que tu veux que la primaire lise.
            },
            onRestoreRequest = {
...
Les deux callbacks portent l'IPC. Ils reçoivent le même `Path` (le fichier restore-request), donc côté secondaire tu y écris quelque chose (typiquement un URI deep-link via [`DeepLinkHandler.writeUriTo`](/docs/lifecycle/deep-links)), côté primaire tu le relis. Plus qu'un URI ? Sers-toi de ce path comme point de rendez-vous pour le format que tu veux.
...
|---|---|---|
| `isSingleInstance(onRestoreFileCreated, onRestoreRequest)` | `(Path.() -> Unit, Path.() -> Unit) -> Boolean` | Renvoie `true` si tu es la primaire, `false` sinon. |
...
| `lockFileName` / `restoreRequestFileName` | Dérivés | À surcharger uniquement si tu veux des noms fixes. |
