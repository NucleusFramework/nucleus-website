...
- Ou appelle `SingleInstanceManager.isSingleInstance(...)` à la main si on n'utilises pas `nucleusApplication { }`.
...
À la main, quand on câbles le propre `application { }` :
    ...
    val isSingle = remember {
        SingleInstanceManager.isSingleInstance(
            onRestoreFileCreated = {
                ...
                // Écris ce que on veux que la primaire lise.
            },
            onRestoreRequest = {
...
Les deux callbacks portent l'IPC. Ils reçoivent le même `Path` (le fichier restore-request), donc côté secondaire on y écris quelque chose (typiquement un URI deep-link via [`DeepLinkHandler.writeUriTo`](/docs/lifecycle/deep-links)), côté primaire on le relis. Plus qu'un URI ? Sers-toi de ce path comme point de rendez-vous pour le format que on veux.
...
|---|---|---|
| `isSingleInstance(onRestoreFileCreated, onRestoreRequest)` | `(Path.() -> Unit, Path.() -> Unit) -> Boolean` | Renvoie `true` si on es la primaire, `false` sinon. |
...
| `lockFileName` / `restoreRequestFileName` | Dérivés | À surcharger uniquement si on veux des noms fixes. |
