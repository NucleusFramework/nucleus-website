...
description: Garantissez une seule instance de votre app et transférez les arguments d'un second lancement vers le process principal.
---
...
- Désactivez avec `nucleusApplication(args, enableSingleInstance = false)`.
...
- Si le fichier de verrou est périmé ou illisible, `SingleInstanceManager` le supprime et réessaie
  une fois. Si le verrou est encore tenu au second essai, le process est une secondaire et envoie
  une restore-request. Seule une erreur irrécupérable le fait basculer en primaire.

## Et ensuite

- [Deep links](/docs/lifecycle/deep-links) — transférez les URL `myapp://...` d'un second lancement vers la primaire.
