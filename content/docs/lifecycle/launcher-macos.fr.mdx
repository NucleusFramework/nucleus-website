...
- Les callbacks de clic arrivent sur l'EDT — on peux modifier l'état Compose directement.
...
Le bridge natif swizzle `applicationDockMenu:` sur le `NSApplicationDelegate` actif lors du premier `setItems(...)`. À partir de là, AppKit demande le menu à Nucleus à chaque clic droit sur le Dock. La construction tourne sur le main thread via `dispatch_sync` ; les callbacks de clic repartent sur l'EDT via `SwingUtilities.invokeLater`, donc on peux muter l'état Compose directement.
