...
description: Remplis le menu clic droit du Dock d'une app macOS avec des items, des sous-menus et des séparateurs, et gère les clics depuis Kotlin.
---

import { Callout } from 'fumadocs-ui/components/callout';

`launcher-macos` te permet de remplir le menu clic droit du Dock d'une app macOS depuis Kotlin.
Tu définis les entrées du menu — sous-menus, séparateurs et items désactivés compris — et tu
reçois un callback sur l'EDT Swing lorsque l'utilisateur clique sur l'une d'elles.
...
Appelle `MacOsDockMenu.setDockMenu(...)` avec les items à afficher, puis définis un `listener`
...
Appelle `MacOsDockMenu.clearDockMenu()` pour retirer le menu et restaurer le comportement par
défaut du Dock.

<Callout type="info">
Toutes les méthodes sont sans effet hors macOS et lorsque la bibliothèque native ne se charge
pas. Vérifie `MacOsDockMenu.isAvailable` avant de construire le menu.
</Callout>
...
Les callbacks de clic sont livrés sur l'EDT Swing : tu peux donc muter l'état Compose
...
- Teste sur un build packagé ou `runDistributable`. L'icône `java` affichée par `./gradlew run`
  n'a pas de bundle identifier et n'affichera pas le menu correctement.
- ProGuard / GraalVM : garde `dev.nucleusframework.launcher.macos.NativeMacOsDockMenuBridge`
