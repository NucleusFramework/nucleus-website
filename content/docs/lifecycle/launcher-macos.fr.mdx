...
L'icône du Dock macOS a un menu clic-droit que personne d'autre ne peut peupler pour toi — et la plupart des apps se contentent de "Quitter / Options / Afficher toutes les fenêtres". `launcher-macos` te permet d'y ajouter des entrées structurées, des sous-menus, des items désactivés, avec des callbacks de clic sur l'EDT.
...
- Les callbacks de clic arrivent sur l'EDT — tu peux modifier l'état Compose directement.
...
Le bridge natif swizzle `applicationDockMenu:` sur le `NSApplicationDelegate` actif lors du premier `setItems(...)`. À partir de là, AppKit demande le menu à Nucleus à chaque clic droit sur le Dock. La construction tourne sur le main thread via `dispatch_sync` ; les callbacks de clic repartent sur l'EDT via `SwingUtilities.invokeLater`, donc tu peux muter l'état Compose directement.
...
- Teste sur un build packagé (ou `runDistributable`). L'icône `java` affichée par `./gradlew run` n'a pas ton bundle id et ne montrera pas le menu correctement.
