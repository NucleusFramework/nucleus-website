...
- **`WindowsBackdrop` — Windows 11 system backdrop** (#412) — a new composable in
  `decorated-window-tao` applies a DWM Mica, MicaAlt, or Acrylic material behind a `DecoratedWindow`
  on the Tao backend. DWM composites the material and it shows wherever Compose paints nothing. The
  `WindowsBackdropStyle` enum (`Default`, `None`, `Mica`, `Acrylic`, `MicaAlt`) carries explicit
  native values, and `WindowsBackdropTier` (`Auto`, `Modern`, `LegacyMica`, `Windows10Acrylic`) pins
  one of the three DWM implementations so each can be tested on any Windows 11 machine. An app
  `tint` layer composites over the material; left unspecified, Mica gets no tint and Acrylic follows
  the window background at a legible opacity. `WindowBackground` is suspended while a backdrop is
  active and restored on dispose, so the backdrop is runtime-toggleable. Older Windows versions
  degrade (legacy Mica attribute on Win11 &lt;22H2, undocumented accent acrylic on Win10) rather than
  no-op. Silent no-op on macOS and Linux. See [Windows backdrop](/docs/tao/windows-backdrop).

### Bug fixes

- **No dark flash on close under a backdrop** (#412) — the close animation snapshots the window
  as-is, and a backdrop's transparent tint composited towards black during the fade-out (worst on
  light themes). `prepareClose` now flips the transparent state off before the close render so the
  fade-out snapshots an opaque themed window. Also fixes caption-button double-hit and Snap Layouts
  via `CaptionButtonHitZones`.
